...
public final class CVCalendarView: UIView {
    ...
    public var changedCurrentDate: Date? {
        get {
            if let delegate = delegate, let date = delegate.changeSelctedDate?() {
                return date
            }
            return nil
        }
...
extension CVCalendarView {
    ...
    public func changeMode(_ mode: CalendarMode,selectedDate:Date = Date(), completion: @escaping () -> () = {}) {
        ...
        case .weekView:
            ...
            newController = WeekContentViewController(calendarView: self, frame: bounds,
                                                      presentedDate: selectedDate1 ?? Date())
        case .monthView:
